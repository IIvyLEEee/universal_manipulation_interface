...
import diffusion_policy.module.quant_linear as qu
import diffusion_policy.module.quant_feedforward as fe
import diffusion_policy.module.quant_attn as qu_attn
...
class TransformerForDiffusion(ModuleAttrMixin):
    def __init__(
        ...
        if T_cond > 0:
            ...
            self.encoder = nn.Sequential(
                Linear(n_emb, 4 * n_emb), nn.Mish(), Linear(4 * n_emb, n_emb)
            )
            # decoder
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, qu.Linear):
            ...
            if module.bias is not None:
                torch.nn.init.zeros_(module.bias)
        elif isinstance(module, fe.Linear):
            torch.nn.init.normal_(module.weight, mean=0.0, std=0.02)
            if module.bias is not None:
                torch.nn.init.zeros_(module.bias)
    ...
    def get_optim_groups(self, weight_decay: float = 1e-3):
        ...
        blacklist_weight_modules = (LayerNorm, torch.nn.Embedding, torch.nn.LayerNorm)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.encoder_only:
            ...
            if self.obs_as_cond:
                cond_obs_emb = self.cond_obs_emb(cond, init)
                # print(cond.max(), cond.min())
                # (B,To,n_emb)
